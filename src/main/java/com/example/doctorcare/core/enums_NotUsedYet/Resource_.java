 ...
 * <p>
 * Enum này được chú thích với {@link com.example.doctorcare.core.processor.annotation.GeneratePermissionConstants}
 * để tự động sinh ra lớp hằng số {@code vn.look.core.security.Permissions}.
 */
@JsonFormat(shape = JsonFormat.Shape.OBJECT)
public enum Resource_ {
    ...
    INDIVIDUAL("individual", "Đầu tư cá nhân", 
        EnumSet.of(Action.MANAGE, Action.LIST, Action.CREATE, Action.VIEW, Action.EDIT, Action.DELETE), 
    ...
    INSTITUTIONAL_DELEGATION("institutional_delegation", "Ủy quyền",
        EnumSet.of(Action.MANAGE, Action.LIST, Action.DELEGATE, Action.VIEW, Action.EDIT, Action.DELETE),
        EnumSet.of(Scope.ANY, Scope.OWN, Scope.ENTERPRISE)),

    /**
     * Ông To nhất có thể ủy quyền cho các thành viên trong tổ chức.
     * Quản lý các thành viên (người dùng) được doanh nhân ủy quyền hỗ trợ.
     * Ví dụ: Doanh nhân A ủy quyền cho Nhân viên B.
     * `INSTITUTIONAL_MEMBER` đại diện cho bản ghi liên kết giữa A và B.
     */
    INSTITUTIONAL_MEMBER("institutional_member", "Thành viên ủy quyền trong tổ chức đầu tư",
    ...
    Resource_(String key, String description, Set<Action> validActions, Set<Scope> validScopes) {
        this.key = key;
